...
from cz_eli_mcp.coverage import build_coverage
from cz_eli_mcp.server import cz_coverage


def _coverage():
    fn = getattr(cz_coverage, "fn", cz_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CZ_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("cz_coverage" in line for line in lines), (
        "an audit entry was written but it does not name cz_coverage"
    )
